...
let num1 = new bI.BigInt('128');
let num2 = new bI.BigInt('25381521125');
...
function factorial2(num1) {
    let temp = num1.abs();
    let temp2 = new bI.BigInt('1');
    let zerosObject = new bI.BigInt('0');
    let onesObject = new bI.BigInt('1');
    let n = parseInt(temp.toString());
    let originalNumber = n;
    console.log('Time Factorial function takes for the first 50 numbers');

    while (!temp.equal(zerosObject)) {
        if (n % 50 === 0 && n !== originalNumber) {
            console.time('50numbers');
        }
        
        temp2 = temp.multiply(temp2);
        if (n % 50 === 0 && n !== originalNumber) {
            console.timeEnd('50numbers');
        }
        temp = temp.subtract(onesObject);
        n--;
    }
...
console.log("Factorial of num1: " + factorial(num1).toString());
console.timeEnd('Factorial');

// Factorial Compute
function factorial(num1) {

    if (num1.equal(new bI.BigInt('1'))) {
        return new bI.BigInt('1');
    }

    return num1.multiply(factorial(num1.subtract(new bI.BigInt('1'))));
}


// console.time('factorial2');
// console.log('Factorial of num1: ' + factorial2(num1));
